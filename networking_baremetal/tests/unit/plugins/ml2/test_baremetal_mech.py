...
class TestBaremetalMechDriver(base.TestCase):
    ...
    def test_initialize(self):
        self.assertEqual(['baremetal'], self.driver.supported_vnic_types)
        self.assertEqual(['flat'], self.driver.supported_network_types)
        self.assertEqual('other', self.driver.vif_type)
    ...
    def test_update_port_postcommit_unsupported_vnic_type_not_bound(
        ...
        m_pc.current = ml2_utils.get_test_port(
            network_id=m_nc.current['id'], vnic_type='mcvtap',
            vif_type='other')
        m_pc.network = m_nc.current
    ...
    def test_update_port_postcommit_supported_vnic_type_bound(
        ...
        m_pc = mock.create_autospec(driver_context.PortContext)
        m_pc.current = ml2_utils.get_test_port(
            network_id=m_nc.current['id'], vnic_type='baremetal',
            vif_type='other')
    ...
    def test_bind_port_unsupported_network_type(self, mpb_pc):
        m_nc = mock.create_autospec(driver_context.NetworkContext)
        m_nc.current = ml2_utils.get_test_network(network_type='vxlan')
        ...
        m_pc.segments_to_bind = [
            ml2_utils.get_test_segment(network_type='vxlan')]
    ...
    def test_bind_port_unsupported_vnic_type(self, mpb_pc):
        m_nc = mock.create_autospec(driver_context.NetworkContext)
        m_nc.current = ml2_utils.get_test_network(network_type='flat')

        m_pc = mock.create_autospec(driver_context.PortContext)
        m_pc.current = ml2_utils.get_test_port(
            network_id=m_nc.current['id'], vnic_type='other')
        m_pc.network = m_nc.current
        m_pc.segments_to_bind = [
            ml2_utils.get_test_segment(network_type='flat')]
    ...
    def test_bind_port(self, mpb_pc):
        ...
        self.driver.bind_port(m_pc)
        m_pc.set_binding.assert_called_once_with(
            m_pc.segments_to_bind[0]['id'], 'other', {})
        mpb_pc.assert_called_once_with('plugin_context', m_pc.current['id'],
                                       'port', 'BAREMETAL_DRV_ENTITIY')
